...
class NeuralNetwork():
    def __init__(self, matrix_input, output, array_layer, loading_file, regularisation, alpha):
        ...
        self.neural_matrix = [np.hstack((self.matrix_ones,
                                        np.float32(matrix_input)))]
    ...
    def __create_weight_matrix__(self, array_layer, loading_file):
        print(loading_file)
        """
        Create the weight matrices in the neural network
        """
        if loading_file == "":
            self.weight_matrix = []
            for i in range(1, len(array_layer)):
                ...
                self.weight_matrix.append(np.float32(np.random.uniform(
                    -min_max, min_max, (previous + 1, actual))))
        else:
            self.weight_matrix = np.load(loading_file, allow_pickle=True)

    def forward_propagation(self):
        """
        Do the forward propagation in the Neural Network
        """
        for i, matrix_weight in enumerate(self.weight_matrix):
            new_matrix = (0.99998 / (1 + np.exp(-np.clip(
                np.dot(self.neural_matrix[i], matrix_weight), -50, 50)))
                          + 0.00001)
    ...
    def cost_function(self):
        ...
        actual_output = np.clip(actual_output, self.epsilon,
                                1 - self.epsilon, actual_output)
        self.cost = (-np.sum(np.multiply(self.output, np.log(actual_output))
                + np.multiply((1 - self.output), np.log(1 - actual_output))))
    ...
    def regularized_cost_function(self):
        ...
        if self.dynamic_alpha == 1:
            self.alpha = np.float32(0.5 / (self.regularized_cost + 0.1))
    ...
    def run(self, nbr_iteration, saving_file):
        ...
        try:
            ...
            if saving_file == "":
                saving_file = input("Enter saving file for weights, press enter if you dont want to save : ")
            else:
                answer = input("Would you still like to save the new weights ? (y) or (n) ? :")
                if answer == 'n':
                    saving_file = ""
        if saving_file != "":
...
def get_args():
    ...
    parser.add_argument("input", type=str, help="The name of the input file.\
                    It need to be stored so that np.load\
                    can properly load the input")
    parser.add_argument("output", type=str, help="The name of the expected \
...
if __name__ == '__main__':
    ...
    output_matrix = np.load(args.output)
    network = NeuralNetwork(input_matrix, output_matrix,
                            args.size_layer, args.load,
                            np.float32(args.regularisation), args.alpha)
